...
// Khai báo tên đăng nhập, mật khẩu
...
// Đặt tên điểm điều khiển
char nodeName[] = "NhapMon";
...
// Khai báo chât kết nối với switch
...
// LED wifi và nút reset
static uint8_t wifiLed      = 2;   //D2
static uint8_t gpio_reset   = 0;
//Chân IR, DHT11 và LDR
...
//Gốc cho các cảm biến
float temperature1 = 0;
float humidity1   = 0;
float ldrVal  = 0;
//Khởi động cảm biến trong DHT library 
DHT dht(DHTPIN, DHT11);
//Giống cái trên
IRrecv irrecv(IR_RECV_PIN);
// decode IR signals nhận từ IR remote
decode_results results;
//Tạo timer để tạo schedule
SimpleTimer Timer;

//Khởi động các công tắc và cảm biến
...
// Callback function để xử lý các thao tác ghi, trên các tham số của các thiết bị
...
{
    // Đọc GPIO0(nút boot)
    if(digitalRead(gpio_reset) == LOW) { //Bấm
        ...
        if ((endTime - startTime) > 10000) {
          // Nếu giữ 10s, reset hết
          Serial.printf("Reset to factory.\n");
          RMakerFactoryReset(2);
        } else if ((endTime - startTime) > 3000) {
          Serial.printf("Reset Wi-Fi.\n");
          // Nếu giữ 3s, reset wifi
          RMakerWiFiReset(2);
        }
    }
    delay(100);
